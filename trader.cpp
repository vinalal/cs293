        ...
        for(int i=0;i<trade_offers.size();i++){
            ...
            if(!flag1){     // add t to cancel_check and subsets
                ...
                for(int j=0;j<size_s;j++){
                    ...
                    subsets.push_back(std::make_pair(m,v));    // pushing each subset after adding t in it to the subsets vector
                ...
                if(arbitrage.empty()){
                    ...
                    for(int i=0;i<arbitrage.size();i++){
                        for(int j=0;j<subsets.size();j++){
                            if(subsets[j].first[arbitrage[i]]){
                                subsets.erase(subsets.begin()+j);
                            }
                        }
                        cancel_check.erase(arbitrage[i]);
                    }
                }
                ...
                int max_profit = 0;                         // add t to cancel_check and subsets
                int profit = 0;
                std::vector<trade_order*> arbitrage;
                bool flag2;
                int size_s = subsets.size();
